...
export class FakeEngine implements TorrentEngine {
  ...
  reads: Array<{ offset: number; length: number; priority?: Priority }> = [];
  hints: Array<{ offset: number; length: number; priority: Priority }> = [];
  aborted = 0;
  destroyed = false;
  ...
  /** Release every read currently waiting. */
  flush(): void {
